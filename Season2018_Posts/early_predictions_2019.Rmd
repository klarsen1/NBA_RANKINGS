...
all_rankings <- read.csv(f) %>%
  mutate(elastic_ranking=min_rank(season_win_rate),
         ...
         absdiff=ifelse(abs(elastic_ranking-FiveThirtyEight)>3, 0, 1)) %>%
...
d <- read.csv(f, stringsAsFactors = FALSE) %>%
  ...
  mutate(total=roster+circumstances+performance) %>%
  arrange(total) %>%
  ungroup() %>%
  ...
  ggplot(aes(x=reorder(team, total), value)) + geom_bar(aes(fill=modelpart), stat="identity") + coord_flip() +
