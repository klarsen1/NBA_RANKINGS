assign_clusters <- function(centroids, data, cutoff, thisseason){
  ...
  scrubs <- filter(means, minutes<=cutoff) %>% mutate(Cluster=0) %>% dplyr::select(PLAYER_FULL_NAME, Cluster)
  means_no_scrubs <- filter(means, minutes>cutoff) %>% dplyr::select(-PLAYER_FULL_NAME, -minutes)
