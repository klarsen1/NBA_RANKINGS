...
* First, a not-so-contoversial. The Golden State Warriors won the title in every single simulation, barely losing any games along the way. 
* Moreinterestingly, howevever, is that the model picks the Boston Celtics as Golden State's opponent in the finals. The model predicts that the Celtics will dispose the Cavs in five games
...
* The results described in the following section reflect the most commonly occuring outcomes.
...
The answer these questions, we can decompose the playoff preductions across all simulations. 
...
* The red bars show sthat circumstanes -- e.g., traveling -- does not matter much for the playoffs.
...
* The model favors Boston over Cleveland due to Cleveland's recent poor performmance (illustrated by the green bars). Cleveland does well in terms of the quality and diversity of the roster, but not in terms of performance. But we all know that this can change in the playoffs.
...
The only thing I know is that all models are wrong. Having said that, barring any injuries, I feel confident that Golden State will reclaim the trophy this year. It seems like a safe prediction and the model seems to agree. 

As mentioned above, we always need to skeptical of models. Often, we must intervene because we know something the model doesn't know. For exapmple, despite what the model says, I think (and hope) we'll see a match-up against Cleveland -- the recent performance we've seen from Cleveland is not indicative of what they can do in the playoffs. 
