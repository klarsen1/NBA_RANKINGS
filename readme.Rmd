...
####Team 1:
For a given game, \\( X_1 = \\)  % of minutes allocated to cluster 1, \\( X_{2} = \\)  % of minutes allocated to cluster 2, etc.

  
####Team 2:
For a given game, \\( Z_1 = \\)  % of minutes allocated to cluster 1, \\( Z_{2} = \\)  % of minutes allocated to cluster 2, etc.

From these variables we construct the “delta variables” given by
 
\\( D_{1} = X_{1} – Z_{1}, D_{2} = X_{2} – Z_{2} \\), etc.

These variable are then directly entered into the logistic regression model (labeled as share_minutes_cluster_XX). The regression model then chooses the importance of each archetype. Hence, for team 1 to have a strong roster compared team 2 -- according to the model -- it must have a surplus of minutes allocated to archetypes with large and positive coefficients, and vice versa for archetypes with negative coefficients.
...
### Player Interaction
The model currently does not cap
