...
When Donald Trump won the 2016 presidential election, Democrats were shocked. Heck, even Republicans were shocked. The election models were wrong and the Nate Silvers of the world took some heat for that (although Nate Silver himself got pretty close). After this, a lot of people would probably agree that the world doesn't need another statistical prediction model.
...
The game level accuracy for the entire period was 64.5% -- i.e., the model predicted the correct winner for 64.5% of games.
...
Checking the game-level predictions. Note that the model is most accurate for the "tail teams" -- which is to be expected. The chart below shows the game-level accuracy for each team after subtracting 50% (the accuracy of a random draw).
...
mutate(game_level, match=as.numeric(selected_team_win==d_pred_selected_team_win)) %>%
  ...
  xlab("Accuracy - 50%") + ylab("") + theme(legend.title = element_blank()) + 
