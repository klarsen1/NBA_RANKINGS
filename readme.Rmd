---
title: "I'd Rather Predict NBA Games Than Elections -- A New Statistical Approach"
...
When Donald Trump won the 2016 presidential election, Democrats were shocked, Republicans were shocked. The Nate Silvers of the world took some heat because the election models were wrong (although Nate Silver himself got pretty close). Most people would probably agree that the world does not need another statistical prediction model.
...
This post discusses a new and data driven approach to predicting the outcome of NBA games. The model is based on techniques frequently used across various industries to predict bankruptcy, fraud or customer buying behavior. While it's certain that the model is wrong -- all models are -- I'm hoping that it will also be useful.
...
The model was used to predict all games from 2015-11-17 to the end the 2015-2016 season, using only information available as of 2015-11-16 (including model coefficients). Most teams played 71 games during this period.
...
The predicted team rankings were also in line with the actual team rankings (correlation is 75%), except for some interesting misses like the Chicago Bulls (actual ranking #14, predicted #4). Note that this table only covers the games played during the 2015-2016 season after the date of 2015-11-16.
...
Checking the game-level predictions. Note that the model is most accurate for the "tail teams" -- which is to be expected:
...
hits <- mutate(gamel_level, match=as.numeric(selected_team_win==d_pred_selected_team_win)) %>%
  mutate(overall_match_percent=mean(match)) %>%
  rename(team=selected_team) %>%
  group_by(team) %>%
  summarise(team_match_percent=mean(match), 
            overall_match_percent=mean(overall_match_percent))
